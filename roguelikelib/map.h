...
namespace RL {
	struct Position {
		...
		bool operator!=(const Position& r) { return !operator==(r); }
	};
	...
	struct SRoom {		
		...
		int type;
		inline bool IsInRoom(const Position &pos) { return (pos.x>=corner1.x && pos.x<=corner2.x && pos.y>=corner1.y && pos.y<=corner2.y); };
